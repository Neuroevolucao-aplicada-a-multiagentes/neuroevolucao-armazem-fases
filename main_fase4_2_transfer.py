...
class Agente:
    ...
    def mover(self, dt):
        ...
        direcao_alvo = pygame.Vector2(alvo.x - self.pos.x, alvo.y - self.pos.y)

        if direcao_alvo.length() > 0 and self.vel.length() > 0:
            alinhamento = self.vel.normalize().dot(direcao_alvo.normalize())
            self.fitness += alinhamento * 0.05
...
while running:
    ...
    screen.blit(
        font_sm.render("START", True, (100, 100, 140)),
        (int(START_POS.x) + 10, int(START_POS.y) - 8)
    )
    ...
    pygame.draw.rect(
        screen,
        (100, 180, 255),
        pygame.Rect(int(PACOTE_POS.x) - 8, int(PACOTE_POS.y) - 8, 16, 16),
        2
    )
    screen.blit(
        font_sm.render("PACOTE", True, (100, 180, 255)),
        (int(PACOTE_POS.x) + 10, int(PACOTE_POS.y) - 8)
    )

    pygame.draw.circle(screen, (220, 80, 80), ENTREGA_POS, RAIO_ENTREGA, 2)
    pygame.draw.circle(screen, (255, 120, 120), ENTREGA_POS, 6)
    screen.blit(
        font_sm.render("ENTREGA", True, (255, 120, 120)),
        (int(ENTREGA_POS.x) + 8, int(ENTREGA_POS.y) - 8)
    )
    ...
    for i, texto in enumerate(linhas):
        screen.blit(
            font_sm.render(texto, True, (230, 230, 230)),
            (12, 10 + i * 22)
        )
    ...
    if historico_fitness:
        ...
        screen.blit(
            font_sm.render("Fitness / Coletas / Entregas", True, (160, 160, 180)),
            (gx + 6, gy + 6)
        )
